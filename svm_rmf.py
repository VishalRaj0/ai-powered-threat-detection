...
train_df = pd.read_csv("KDDTrain+.txt", names=col_names).iloc[:, :-1]
test_df = pd.read_csv("KDDTest+.txt", names=col_names).iloc[:, :-1]
...
rf_search = GridSearchCV(RandomForestClassifier(random_state=42), param_grid_rf, cv=3, scoring='accuracy', n_jobs=-1)
...
stacker2 = StackerTransformer(stack_rf)
...
plt.title("ROC Curve - Stack(SVM + RF) → MLP")
plt.legend()
plt.grid(True)
plt.tight_layout()
plt.show()
